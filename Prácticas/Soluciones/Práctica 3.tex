...
\begin{enumerate}[label=(\alph*)]
	...
	\item	\begin{proc}{interseccion}{\In s, t: \TLista{T}, \Out res: \TLista{T}}{}
			\pre{True}
			\post{(\forall elem: T)(\#apariciones(res, elem) = min(\#apariciones(s, elem), \#apariciones(t, elem)))}
			\end{proc}
...
\begin{enumerate}
    \item \begin{proc}{cantApariciones}{\In l: String, \Out res: \TLista{Char \times \ent}}{}
    	\pre{True}
    	\post{mismosElementosEnDict(l, res) \y sinKeysRepetidos(res) \y \\
    		(\forall i: \ent)(0 \leq i < \longitud{res} \implicaLuego res[i]_{1} = \#apariciones(l, res[i]_{0}))}
    \end{proc}

	\pred{mismosElementosEnDict}{s: \TLista{T}, d: \TLista{T \times U}}
	{(\forall elem: T)((elem \in s \y existeKey(elem, d)) \lor (elem \notin s \y \neg existeKey(elem, d)))}
	
	\pred{existeKey}{elem: T, d: \TLista{T \times U}}
	{(\exists i: \ent)(0 \leq i < \longitud{d} \yLuego d[i]_0 = elem)}
	
	\pred{sinKeysRepetidos}{d: \TLista{T \times U}}
	{(\forall i,j \ent)((0 \leq i,j < \longitud{d} \y i \neq j) \implicaLuego d[i]_0 \neq d[j]_0)}
    \item \begin{proc}{prefijos}{\In s: \TLista{T}, \Out res: \TLista{\TLista{T}}}{}
    	\pre{True}
    	\post{\longitud{res} = \longitud{s} + 1 \yLuego (\forall i: \ent)(0 \leq i < \longitud{res} \implicaLuego res[i] = subseq(s, 0, i))}
    \end{proc}
    \item   \begin{proc}{secuenciaConElMayorValor}{\In s: \TLista{\TLista{\ent}}, \Out res: \TLista{\ent}}{}
            \pre{\longitud{s} > 0}
            \post{res \in s \y (\forall l: \TLista{\ent})(l \in s \implica contieneElMayorValorEntre(res, l))} 
            \end{proc}
        
        \pred{contieneElMayorValorEntre}{s, t: \TLista{\ent}}
